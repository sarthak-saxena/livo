...
var CallPad = function (_a) {
    ...
    var raiseHand = function () {
        ...
        if (value) {
            ...
            App_1.dataStore.update(Voxeet_1.VoxeetCommandType.RaiseHand, participantId);
        } else {
            sdk_1.unRaiseHandInConference(participantId);
            App_1.dataStore.update(Voxeet_1.VoxeetCommandType.unRaiseHand, participantId);
        }
        setHandRaised(value);
    };
    var attendee = userHook_1.useAttendee().attendee;
    var _b = useDataFromDataSync(participantId),
        setHandRaisedDefault = _b.setHandRaisedDefault,
        muteMikeDefault = _b.muteMikeDefault,
        enableMikeDefault = _b.enableMikeDefault,
        requestSpeakerAccessButtonEnabledDefault = _b.requestSpeakerAccessButtonEnabledDefault;
    var classes = useStylesFromThemeFunction(props);
    var _c = react_1.useState(setHandRaisedDefault),
        isHandRaised = _c[0],
        setHandRaised = _c[1];
    var _d = react_1.useState(muteMikeDefault),
        isMikeMute = _d[0],
        muteMike = _d[1];
    var _e = react_1.useState(enableMikeDefault),
        isMikeEnabled = _e[0],
        enableMike = _e[1];
    var _f = react_1.useState(requestSpeakerAccessButtonEnabledDefault),
        requestSpeakerAccessButtonEnabled = _f[0],
        enableRequestSpeakerAccessButton = _f[1];
